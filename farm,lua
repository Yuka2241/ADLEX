...
local function uAt(s)
    ...
    -- ПРИГЛУШЁННЫЙ ЗЕЛЁНЫЙ для ON состояния
...
local Themes = {
    Black = {bg = Color3.fromRGB(15, 15, 18), side = Color3.fromRGB(22, 22, 26), text = Color3.fromRGB(255, 255, 255), logoText = Color3.fromRGB(255, 255, 255), wmText = Color3.fromRGB(240, 240, 245)},
    Gray = {bg = Color3.fromRGB(35, 35, 40), side = Color3.fromRGB(45, 45, 50), text = Color3.fromRGB(240, 240, 245), logoText = Color3.fromRGB(255, 255, 255), wmText = Color3.fromRGB(240, 240, 245)},
    Light = {bg = Color3.fromRGB(240, 240, 245), side = Color3.fromRGB(225, 225, 230), text = Color3.fromRGB(20, 20, 25), logoText = Color3.fromRGB(20, 20, 25), wmText = Color3.fromRGB(20, 20, 25)}
}
...
-- ИСПРАВЛЕНО: Функция обновления текста кнопки Menu Key
local function updateMenuKeyUI()
    if _G.tabs["Settings"] and _G.tabs["Settings"].f then
        ...
        if btn then
            btn.Text = "Menu Key: [" .. _G.menuKey.Name .. "]"
        end
    end
end
...
-- Кнопка Menu Key с именем для поиска
local mkb = _G.cBtn(_G.p3, "Menu Key: [RightShift]", UDim2.new(0, 0, 0, 205), UDim2.new(0, 200, 0, 35), Color3.fromRGB(114, 9, 183), "Изменить кнопку меню")
mkb.Name = "MKeyBtn"
mkb.MouseButton1Click:Connect(function()
    _G.menuBinding = true
    _G.bindOverlay.Text = "Нажмите клавишу для закрытия меню..."
    _G.bindOverlay.Visible = true
    -- Показываем текущую клавишу в оверлее
    showToast("Binding", "Текущая: " .. _G.menuKey.Name)
end)
-- Оверлей для бинда клавиш
...
local boLabel = Instance.new("TextLabel", _G.bindOverlay)
boLabel.Size = UDim2.new(1, 0, 1, 0)
boLabel.BackgroundTransparency = 1
boLabel.TextColor3 = Color3.fromRGB(240, 240, 245)
boLabel.Font = Enum.Font.GothamBold
boLabel.TextSize = 14
boLabel.ZIndex = 201
game:GetService("UserInputService").InputBegan:Connect(function(i, g)
    if _G.flyBinding and i.KeyCode ~= Enum.KeyCode.Unknown and i.UserInputType == Enum.UserInputType.Keyboard then
        ...
        _G.bindOverlay.Visible = false
        uFlV()
        showToast("Fly Key Set", "Клавиша полёта: " .. _G.flyKey.Name)
        return
    end
    if _G.menuBinding and i.KeyCode ~= Enum.KeyCode.Unknown and i.UserInputType == Enum.UserInputType.Keyboard then
        ...
        _G.bindOverlay.Visible = false
        updateMenuKeyUI()
        showToast("Menu Key Set", "Клавиша меню: " .. _G.menuKey.Name)
        return
    end
    if g then return end
    if i.KeyCode == _G.flyKey then tgFl() end
    if i.KeyCode == _G.menuKey then _G.mf.Visible = not _G.mf.Visible end
end)
game:GetService("TestService"):Message("[Adlex]: Loaded Adlex.lua")
